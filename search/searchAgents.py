...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        isGoalState = state[0] and state[1] and state[2] and state[3]
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            if(not self.walls[x][y]):
                ...
                newState = (cornerMap[0], cornerMap[1], cornerMap[2], cornerMap[3], (x, y))
    ...
    while(not finished):
        ...
        for index in range(0, len(corners)):
            if(not visited[index]):
                distance = abs(corners[index][0] - current[0])  + abs(corners[index][1] - current[1])
...
def foodHeuristic(state, problem):
    ...
    for index in range(len(foodList)):
        dist += (abs(foodList[index][0] - current[0]) + abs(foodList[index][1] - current[1]))
    if(len(foodList) > 0):
        return dist / len(foodList)
    else:
        return 0
