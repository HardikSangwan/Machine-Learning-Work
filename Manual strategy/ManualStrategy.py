...
def testPolicy(symbol="AAPL", sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011, 12, 31), sv=100000):
    ...
    for day in pd.date_range(sd, ed):
        if (a.loc[day, "Price/SMA Ratio"]<1.1 and a.loc[day, 'Bollinger Band Percent']<0 and (0>a.loc[day, 'Momentum'] and a.loc[last, 'Momentum']))and holdings <1000:
            orders.append(1000-holdings)
            holdings = 1000
        elif (a.loc[day, "Price/SMA Ratio"]>.9 and a.loc[day, 'Bollinger Band Percent']>1 and (0<a.loc[day, 'Momentum'] and a.loc[last, 'Momentum']>0))and holdings >-1000:
            orders.append(-1000-holdings)
            holdings = -1000
        else:
            orders.append(0)
...
if __name__ == "__main__":
    ...
    price = get_data(["JPM"], pd.date_range(sd, ed), addSPY=False)
    price.fillna(method='ffill', inplace=True)
    price.fillna(method='bfill', inplace=True)
    price /= price.values[0]
    short = df[df == -1000].index
    longg = df[df == 1000].index
    man = msc.compute_portvals(df, symbol = 'JPM', sd = sd, ed = ed, sv = sv, commission=9.95/price.values[0], impact=0.005)
    df1 = df.copy()
    df1[:] = 0 
    df1.iloc[0] = 1000
