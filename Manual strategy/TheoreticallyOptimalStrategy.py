...
def returndata(data):
    daily_returns = (data[1:] / data[:-1].values) - 1
    cr = (data[-1] / data[0]) - 1
...
if __name__ == "__main__":
    ...
    Bestnorm = Best/Best.iloc[0]
    Benchnorm = Benchmark/Benchmark.iloc[0]
                   
    d = {'Theoretically Optimal': Bestnorm, 'Benchmark': Benchnorm}
    df1 = pd.DataFrame(data = d)
    df1.plot(title ='Theoretically Optimal vs Benchmark', color= ['Blue', 'Black'])
#    plt.plot(Best/Best.iloc[0], "black")  # red line
#    plt.plot(Benchmark/Benchmark.iloc[0], color="blue")  # blue line
#    plt.legend(['Best', 'Benchmark'])
#    plt.savefig('figure4.png')
    
    print returndata(Bestnorm)
    print returndata(Benchnorm) 
